...
SimpleConjugateGradient::SimpleConjugateGradient(BaseFunction &obj, std::vector<Point2<double>> &var, const double &alpha, const vector<bool> &fixed)
    : BaseOptimizer(obj, var), grad_prev_(var.size()), dir_prev_(var.size()), step_(0), alpha_(alpha), fixed_(fixed) {}
...
void SimpleConjugateGradient::Step() {
    ...
    if (step_ == 0) {
        ...
        for (size_t i = 0; i < kNumModule; ++i) {
            Point2<double> t3 = obj_.grad().at(i) * (obj_.grad().at(i) - grad_prev_.at(i));
            t1 += t3.x + t3.y;
